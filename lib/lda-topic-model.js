...
class topicModelling {
  constructor(settings, sentences, sweeps, numTopics, dict, displayingStopWords) {
    ...
    if (!isNaN(numTopics) && numTopics > 0) {
    ...
    if (displayingStopWords) {
      this.displayingStopwords = displayingStopWords;
    } else{
    ...
    this.tokensPerTopic = this.zeros(numTopics);
    this.topicWeights = this.zeros(numTopics);

    this.loadVocabulary();
    this.prepareData(sentences);
    if (sweeps !== undefined) {
      this.requestedSweeps = sweeps;
    } else {
      this.requestedSweeps = 100;
    }
